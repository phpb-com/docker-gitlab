...
> Alternatively image is available from docker hub `gotfix/gitlab:9.0.0-2`
...
```bash
docker pull quay.io/gotfix/gitlab:9.0.0-2
```
...
docker run --name gitlab -d [OPTIONS] \
    quay.io/gotfix/gitlab:9.0.0-2 app:sanitize
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    quay.io/gotfix/gitlab:9.0.0-2 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    quay.io/gotfix/gitlab:9.0.0-2 app:rake gitlab:import:repos
```
...
Update `docker-compose.yml` and change `image:` variable to `quay.io/gotfix/gitlab:9.0.0-2`, to have it look like snippet below:
```yaml
...
  gitlab:
    restart: always
...
For docker:
```bash
docker run --name gitlab -d [OPTIONS] quay.io/gotfix/gitlab:9.0.0-2
```
