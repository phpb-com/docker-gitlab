...
> Alternatively image is available from quay.io `quay.io/gotfix/gitlab:9.0.2-1`
...
docker run --name gitlab -d [OPTIONS] \
    gotfix/gitlab:9.0.2-1 app:sanitize
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gotfix/gitlab:9.0.2-1 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gotfix/gitlab:9.0.2-1 app:rake gitlab:import:repos
```
...
Update `docker-compose.yml` and change `image:` variable to `gotfix/gitlab:9.0.2-1`, to have it look like snippet below:
```yaml
...
  gitlab:
    restart: always
...
For docker:
```bash
docker run --name gitlab -d [OPTIONS] gotfix/gitlab:9.0.2-1
```
