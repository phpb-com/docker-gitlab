...
# gotfix/gitlab:9.0.4-2

> Alternatively image is available from quay.io `quay.io/gotfix/gitlab:9.0.4-2`
...
```bash
docker pull gotfix/gitlab:9.0.4-2
```
...
docker run --name gitlab -d [OPTIONS] \
    gotfix/gitlab:9.0.4-2 app:sanitize
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gotfix/gitlab:9.0.4-2 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gotfix/gitlab:9.0.4-2 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gotfix/gitlab:9.0.4-2 app:rake gitlab:backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gotfix/gitlab:9.0.4-2 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    gotfix/gitlab:9.0.4-2 app:rake gitlab:import:repos
```
...
Update `docker-compose.yml` and change `image:` variable to `gotfix/gitlab:9.0.4-2`, to have it look like snippet below:
```yaml
...
  gitlab:
    restart: always
    image: gotfix/gitlab:9.0.4-2
...
```
...
For docker:
```bash
docker run --name gitlab -d [OPTIONS] gotfix/gitlab:9.0.4-2
```
