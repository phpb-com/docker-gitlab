...
# quay.io/gotfix/gitlab:9.0.1

> Alternatively image is available from docker hub `gotfix/gitlab:9.0.1`
...
Since I do not have any means to test well with MySQL, I decided to let it go. This might change depending on the future and contributions. It would be very trivial to plug it back in, just revert one commit.
...
```bash
docker pull quay.io/gotfix/gitlab:9.0.1
```

You can also pull the `latest` tag which is built from the repository *HEAD*

```bash
docker pull quay.io/gotfix/gitlab:latest
```

Alternatively you can build the image locally.

```bash
docker build -t quay.io/gotfix/gitlab gotfix.com/docker/gitlab
```
...
To illustrate linking with a postgresql container, we will use the [quay.io/gotfix/postgresql](https://gotfix.com/docker/postgresql) image. When using postgresql image in production you should mount a volume for the postgresql data store. Please refer the [README](https://gotfix.com/docker/postgresql/blob/master/README.md) of docker-postgresql for details.
...
Here the image will also automatically fetch the `DB_NAME`, `DB_USER` and `DB_PASS` variables from the postgresql container as they are specified in the `docker run` command for the postgresql container. This is made possible using the magic of docker links and works with the following images:
 ...
 - [quay.io/gotfix/postgresql](https://quay.io/repository/quay.io/gotfix/postgresql/)
...
To illustrate linking with a redis container, we will use the [quay.io/gotfix/redis](https://gotfix.com/docker/redis) image. Please refer the [README](https://gotfix.com/docker/redis/blob/master/README.md) of docker-redis for details.
...
docker run --name gitlab -d [OPTIONS] \
    quay.io/gotfix/gitlab:9.0.1 app:sanitize
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    quay.io/gotfix/gitlab:9.0.1 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    quay.io/gotfix/gitlab:8.14.4 app:rake db:setup
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    quay.io/gotfix/gitlab:9.0.1 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    quay.io/gotfix/gitlab:9.0.1 app:rake gitlab:backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    quay.io/gotfix/gitlab:9.0.1 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    quay.io/gotfix/gitlab:9.0.1 app:rake gitlab:import:repos
```
...
Update `docker-compose.yml` and change `image:` variable to `quay.io/gotfix/gitlab:9.0.1`, to have it look like snippet below:
```yaml
...
  gitlab:
    restart: always
    image: quay.io/gotfix/gitlab:9.0.1
...
```
...
For docker:
```bash
docker run --name gitlab -d [OPTIONS] quay.io/gotfix/gitlab:9.0.1
```
