...
| `OAUTH_SYNC_EMAIL_FROM_PROVIDER` | Sync user's email address from the specified Omniauth provider every time the user logs in. Make this field read-only. Accepted values are `cas3`, `github`, `bitbucket`, `gitlab`, `google_oauth2`, `facebook`, `twitter`, `saml`, `crowd`, `auth0` and `azure_oauth2`. No default. |
